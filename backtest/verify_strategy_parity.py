...
REGIONS = [
    ...
    ("preset -> parameter resolution",
     '// ── resolve the preset', '// ─────────────────────────────────────────────────────────────'),
    ("ATR, trade-day clock, session windows, PO3 period open, VWAP bands",
     'float atrVal = ta.atr(ATR_LEN)', 'fill(p_u1, p_l1,'),
    ("live raid state (one raid per side)",
     '// one live raid per side;', '// running period extremes'),
    ("running period extremes",
     '// running period extremes', '// v6 evaluates and/or lazily'),
    ("hoisted ta.* calls",
