...
def main(data, cost=4.0, tf="1h"):
    ...
    print(f"\n{'=' * 96}\naxis corroboration on {tf}, cost {cost} ticks   "
          f"MNQ = selection market (IN-SAMPLE 60% ONLY, so MNQ OOS stays clean), "
          f"POOL = NQ+ES+YM+RTY\n{'=' * 96}")
    print(f"{'setting':<34} {'MNQ n':>6} {'PF':>6} {'expR':>7}   "
          f"{'POOL n':>6} {'PF':>6} {'expR':>7}  {'sym+':>4}")
    for axis, vals in AXES.items():
        print(f"-- {axis}")
        for v in vals:
            p = replace(CANDIDATE, **{axis: v})
            m = evaluate(cache, p, ["MNQ"], cost, split=(0.0, 0.6))
