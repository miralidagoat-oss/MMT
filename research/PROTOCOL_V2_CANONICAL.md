...
**Behavioural proof.** The 19 winsorization constants were generated by this
rule. `research/EVENT_STREAM_CHECKSUM.json` pins the burn-in event stream
(order, identity and every feature value) at
`cb3a3033…5eab`; `backtest/test_first_penetration.py` proves the state machine
against the shipped engine. If the checksum moves, the freeze is invalid and
must be recomputed.
...
2. feature missingness ≤ 20% — applied **literally**, as frozen before any result: not tightened, not rounded up, given no safety margin. Measured on the burn-in: `dist_overnight_high_atr` and `dist_overnight_low_atr` at **61.99%** are **NON-PROMOTABLE** (predeclared, in the original family, descriptively reportable; no favourable p-value may override a gate failed before outcomes were seen, and neither feature may be repaired, replaced or re-defined). `htf_1h_dist_ref_atr` at **19.80% PASSES**. The two non-promotable features create **no vacancies**: Holm **m = 22** stands, nothing is substituted, and no multiplicity power is gained
